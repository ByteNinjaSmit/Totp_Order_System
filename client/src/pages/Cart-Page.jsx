...
export const Cart = () => {
  ...
  const increaseQuantity = (ID) => {
    setQuantities(prevQuantities => {
      const updatedQuantities = { ...prevQuantities, [ID]: (prevQuantities[ID] || 1) + 1 };
      const updatedCart = cart.map(item => item._id === ID ? { ...item, quantity: updatedQuantities[ID] } : item);
      setCart(updatedCart);
    ...
    return (
      <>
        <div className="bg-slate-400 pt-4 pb-2">
          <h1 className="w-full text-center mt-1 sm:text-5xl text-2xl lg:text-4xl mb-10 font-bold">Your Cart</h1>
        </div>
        <div className="mt-4">
          <h4 className="w-full text-center mt-1 sm:text-3xl text-1xl lg:text-2xl mb-10 font-bold">You Have {cart.length} items in your cart. Please Login To Checkout</h4>
        </div>
      </>
    );
  }
  ...
  if (user && cart.length === 0) {
    return (
      <>
        ...
        <div className="mt-5">
          <h4 className="w-full text-center mt-1 sm:text-3xl text-red-500 text-1xl lg:text-2xl mb-10 font-bold">Cart is Empty</h4>
        </div>
      </>
    );
  }
          ...
          <div className="grid grid-cols-12">
            <div className="col-span-12 xl:col-span-8 lg:pr-8 pt-14 pb-8 lg:py-24 w-full max-xl:max-w-3xl max-xl:mx-auto">
              <div className="flex items-center justify-between pb-8 border-b border-gray-300">
                ...
                <h2 className="font-manrope font-bold text-xl leading-8 text-gray-600">{uniqueCart.length} Items</h2>
              </div>
              ...
              {uniqueCart.map((item, index) => (
                <div key={index} className="flex flex-col md:flex-row items-center justify-between mt-8 pb-6 border-b border-gray-200 group">
                  <div className="w-full md:max-w-[126px]">
                    <img src={item.image} alt={item.name} className="mx-auto rounded-full pr-2" />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-4 w-full mt-4 md:mt-0">
                    <div className="md:col-span-2">
                      <div className="flex flex-col max-w-[500px] items-start gap-3">
                        ...
                        <p className="font-normal text-sm text-wrap leading-7 text-gray-500">{item.ingredients.join(', ')}</p>
                    ...
                    <div className="flex items-center justify-center md:justify-end mt-4 md:mt-0 h-full">
                      <p className="font-bold text-lg leading-8 text-gray-600 text-center transition-all duration-300 group-hover:text-indigo-600">₹{ quantities[item._id] * item.price}</p>
                    </div>
                    <div className="flex items-center justify-center md:justify-end mt-4 md:mt-0 h-full">
                      <button onClick={() => removeItem(item._id)} className="text-white bg-red-500 hover:bg-red-600 px-4 py-2 rounded-md focus:outline-none">Remove</button>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="col-span-12 xl:col-span-4 bg-gray-50 w-full max-xl:px-6 max-w-3xl xl:max-w-lg mx-auto lg:pl-8 py-24">
              <h2 className="font-manrope text-center font-bold text-3xl leading-10 text-black pb-8 border-b border-gray-300">Order Summary</h2>
              <div className="mt-8">
                <div className="flex items-center justify-between py-8">
                  <p className="font-medium text-xl leading-8 text-black">{uniqueCart.length} Items</p>
